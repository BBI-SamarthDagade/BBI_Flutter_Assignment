...
void main() {
  ...
  group('UpdateTaskUseCase', () {
    test('should return void when the task is updated successfully', () async {
      // Arrange
      when(() => mockTaskRepository.updateTask(taskEntity, userId))
          .thenAnswer((_) async => right(null));

      // Act
      final result = await updateTaskUseCase.call(taskEntity, userId);

      // Assert
      expect(result, isA<Right<Failure, void>>());
      verify(() => mockTaskRepository.updateTask(taskEntity, userId)).called(1);
    });

    test('should return a Failure when updating the task fails', () async {
      // Arrange
      when(() => mockTaskRepository.updateTask(taskEntity, userId)).thenAnswer(
          (_) async => left(Failure(message: 'Failed to update task')));
